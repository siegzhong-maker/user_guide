...
|------|--------|----------------------|
| 产品定位 | 首发视频（首次上手）+ 霍尔校准 + 架设说明 + 界面操作蒙层 | 线性多页「录制前说明」；**无内置视频教程** |
...
**首次上手（`tw_proto_onboarding_done_v1` 尚未为 `1`）默认顺序**：全屏首发视频（`falcon-intro`）→ 关闭播放器并写入已看后 → **霍尔校准** → 架设引导（Hall guide）→ UI 蒙层。
...
| 视频教程 | **无**；设置页 Tutorial **不展示**视频预览区与配套说明（仅保留标题、`?` 开关与重播入口），完成轮播后**不会**自动弹出空教程列表 | — |
...
- **Falcon**：首次上手先全屏首发视频，关闭后再霍尔校准 → 架设引导 → UI 蒙层。UI 引导末步勾选「不再展示」后，下次仍走霍尔校准，但不再自动播放架设引导与 UI 蒙层。
- **变色龙**：无视频教程；勾选末页「不再展示」后，进入取景不再自动播四步流程；Tutorial Center 仍可手动打开（列表为空）。
- **两端**：`?` / Tutorial Center 与自动引导开关相互独立。Falcon 不再自动弹出教程列表（避免与首发全屏视频抢焦点）；可通过 `?` 手动打开列表。
